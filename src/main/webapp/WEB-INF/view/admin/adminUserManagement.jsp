													...
													<td>
														<div class="table-data-feature">
															...
															<button class="item change-level" data-toggle="tooltip"
																data-placement="top"
																data-target="#changeLevelModal-${user.levelId}"
																title="관리자권한승급">
															...
															<div class="modal fade"
																id="changeLevelModal-${user.levelId}" tabindex="-1"
																role="dialog" aria-labelledby="changeLevelModalLabel"
																aria-hidden="true" data-backdrop="false">
																<div class="modal-dialog" role="document">
																	<div class="modal-content">
																		<div class="modal-header">
																			<h5 class="modal-title" id="changeLevelModalLabel">Change
																				Level</h5>
																		...
																		<div class="modal-body">
																			...
																			<button class="btn btn-primary"
																				onclick="changeUserLevel(${user.levelId}, 2)">관리자
																				권한</button>
																																							
																			<button class="btn btn-secondary"
																				onclick="changeUserLevel(${user.levelId}, 3)">VIP
																				권한</button>
																				
																			<button class="btn btn-danger"
																				onclick="changeUserLevel(${user.levelId}, 1)">일반유저
																				권한</button>
																		</div>
																	</div>
																</div>
...
function changeUserLevel(levelId, newLevelId) {
    var username = $("#usernameInput").val();

    // Make an AJAX request to update the user's level
    $.ajax({
        type: 'POST',
        url: '/admin/updateUserLevel', // Update this URL to the correct endpoint on your server
        data: {
            username: username,
        ...
        success: function(data) {
            ...
            if (data.result === "success") {
                ...
                // Optionally, you can update the UI to reflect the new level
                $("#levelId").text(newLevelId);
                $("#changeLevelModal-" + levelId).modal("hide");
            } else {
                alert("권한 변경에 실패했습니다.");
            }
            $("#changeLevelModal-" + levelId).modal("hide");
...
$(document).ready(function() {
    ...
    $(".change-level").on("click", function() {
        ...
        $('#changeLevelModal-' + levelId).modal('show');
    });
});
...
$(document).ready(function() {
    ...
    $(".delete-user").on("click", function() {
        ...
        if (confirm("삭제하시겠습니까?")) {
            $.ajax({
                type: 'POST',
                url: '/admin/adminUserDelete/' + username,
