...
<body>
	<div>
		<div id="img-box">
		<img src="${restaurantDetail.imgPath}" onerror="this.src='/images/NoImage.jpg'" id=imgPath>
		</div>
	</div>
	<div id="box">		
		<br>
		<div id="title">${restaurantDetail.title}</div>
		<br>
		<div id="introduction">" ${restaurantDetail.introduction} "</div>
		<br>
		<div id="tag">${restaurantDetail.tag}</div>
		<div><input type="hidden" value="${restaurantDetail.contentsLabel}" id="contentsLabel"></div>
		<br><hr>
	...
	<form name="insertReview" id="insertReview" method="post">
		...
		<div>
			<input type="hidden" id="contentsId" value="${restaurantDetail.contentsId}">
		</div>
...
<script>
	...
	let options = { //지도를 생성할 때 필요한 기본 옵션
		center: new kakao.maps.LatLng("${restaurantDetail.latitude}", "${restaurantDetail.longitude}"), //지도의 중심좌표.
